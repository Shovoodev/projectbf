...
const CallToActionSection = () => {
  ...
  const mobileRegex = useMemo(() => /^[0-9()+\-\s]+$/, []);
  ...
  const validateField = (name, value) => {
    ...
    if (name === "first_name") {
      ...
      else if (!mobileRegex.test(v)) error = "Please enter a valid mobile number.";
    }
  ...
  const handleFieldChange = (e) => {
    ...
    // validate only required fields on change
    if (name !== "message") {
  ...
  }, [formData, fieldErrors, mobileRegex]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (errors.first_name || errors.last_name || errors.mobile) return;

    setStatus({ loading: true, success: null, error: null });

    try {
      if (!CORE) {
        throw new Error("VITE_API_URL is missing. Please set it in your .env file.");
      }

      // Backend expects: firstName, lastName, phone, message
      const payload = {
        firstName: formData.first_name.trim(),
        lastName: formData.last_name.trim(),
        phone: formData.mobile.trim(),
        message: (formData.message || "").trim(), // optional
      };

      const res = await fetch(`${CORE}/call-me`, {
        method: "POST",
      ...
      setFieldErrors({ first_name: "", last_name: "", mobile: "" });
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
          ...
          <div className="flex flex-col justify-center items-center lg:items-start text-center lg:text-left space-y-6 lg:pr-12">
            ...
            <p className="text-gray-600 text-lg">
              Planning a service can be overwhelming, but we’re here to make it simple.
            </p>
            ...
            <div className="flex flex-col items-center gap-4 mt-6 sm:flex-row sm:items-center sm:justify-start">
              ...
              <a href="tel:1300110031" className="text-2xl font-body text-gray-900">
                1300 11 0031
              </a>
            </div>
          </div>
          ...
          <div className="bg-white p-8 rounded-xl shadow-lg border flex flex-col items-center">
            ...
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-4 w-full" noValidate>
              {status.success && (
                <div
              ...
              {status.error && (
                <div
                  className="text-red-700 font-medium p-3 rounded bg-red-50 border border-red-200 text-center"
                  role="alert"
                >
                  {status.error}
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <input
                    ...
                    className={`w-full border p-2 rounded transition-colors ${fieldErrors.first_name ? "border-red-500 bg-red-50" : "border-gray-200"
                      }`}
                  />
                  {fieldErrors.first_name && (
                ...
                <div>
                  <input
                    ...
                    className={`w-full border p-2 rounded transition-colors ${fieldErrors.last_name ? "border-red-500 bg-red-50" : "border-gray-200"
                      }`}
                  />
                  {fieldErrors.last_name && (
              ...
              <div>
                <input
                  ...
                  className={`w-full border p-2 rounded transition-colors ${fieldErrors.mobile ? "border-red-500 bg-red-50" : "border-gray-200"
                    }`}
                />
                {fieldErrors.mobile && (
              ...
              {/* Optional EmailJS template field if you use it */}
              <input type="hidden" name="to_email" value="admin1@email.com,admin2@email.com" />
