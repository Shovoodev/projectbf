...
const PopupEnquirey = ({
  ...
  const handleEnquirySubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    const email = isValidEmail(enquireyData.email)
    const payload = {
      ...enquireyData, email
    }
    try {
      ...
      case "enquirey":
        return (
          <form onSubmit={handleEnquirySubmit} className="space-y-6">
            ...
            {message.text && (
              <div
                className={`mt-4 p-3 rounded-lg text-center font-medium transition-all duration-300 ${message.type === "error"
  ...
  return (
    <>
      ...
      >
        <div
          className={`relative w-full h-full transform rounded-2xl transition-all duration-300 ${isOpen ? "scale-80 opacity-100" : "scale-90 opacity-0"
            }`}
        >
